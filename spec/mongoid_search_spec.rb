...
describe Mongoid::Search do
  ...
  it "should accept conditions for the search" do
    Product.search("apple", :conditions => {:displayed => true}).size.should == 1
  end

  it "should not return results that do not match the search conditions" do
    Product.search("apple", :conditions => {:displayed => false}).size.should == 0
  end
end
